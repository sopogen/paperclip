...
## Mandatory Style: Swiss Modern (locked)

**Every presentation produced by this skill MUST use the Swiss Modern preset (preset #11 in `STYLE_PRESETS.md`).** Do not offer style picks, do not use any other preset, and do not silently mix in other aesthetics — even if the user asks for "a different style", apply Swiss Modern and add a one-line note in your final deliverable comment that the house style was applied.

Swiss Modern (Bauhaus-inspired):

- **Typography:** `Archivo` (weight 800) for display + `Nunito` (weight 400) for body. Use Google Fonts.
- **Colors:** Pure white background, pure black text, red accent `#ff3300`. Avoid extra accent hues.
- **Signature:** Visible grid lines, asymmetric layouts, geometric shapes (rectangles, circles), tight typographic hierarchy. No illustrations, no gradients, no emojis.
- **Motion:** Sparse, precise reveals (geometric translate / scale). Avoid playful bouncing.

When the brief calls for brand colors (e.g. "use SK red"), substitute the red accent only. Keep the white/black structure unchanged.

Skip the discovery / preview-pick phase from the workflow below — go straight to building the deck in Swiss Modern.
...
5. **Never use emojis.** It is not professional. Use text or icons instead.
...
- **Text appears too small in PDF.** The export script captures at 1920×1080, where `clamp()` max values tuned for ~1440px screens render text visually small. The script auto-injects `!important` overrides (~1.5x the standard ranges) on the typography variables `--title-size`, `--h2-size`, `--h3-size`, `--body-size`, `--small-size`, and `--mono-size` before screenshotting. This only works when the presentation uses these standard variable names (which this skill's templates do). If a presentation uses different typography variable names, text will still appear small — either rename the variables to match, or extend the `addStyleTag` block in `export-pdf.sh`. Layout variables (padding, gaps) are intentionally NOT scaled, to avoid content overflow inside `.slide`'s `overflow: hidden` viewport.
