...
// ─── PDF typography boost ────────────────────────────────
// At 1920x1080, clamp() max values in the default template are tuned for
// ~1440px screens, so text renders visually small. Inject a 2x scale-up
// for the common typography variables used by this skill's templates.
// Layout-related variables (padding, gaps) are intentionally left alone
// to avoid content overflow inside .slide's overflow:hidden viewport.
await page.addStyleTag({
  content: `
    :root {
      --title-size: clamp(3rem, 8.25vw, 6.4rem) !important;
      --h2-size: clamp(2rem, 5vw, 3.5rem) !important;
      --h3-size: clamp(1.4rem, 2.9vw, 2rem) !important;
      --body-size: clamp(1.17rem, 2vw, 1.58rem) !important;
      --small-size: clamp(1rem, 1.5vw, 1.2rem) !important;
      --mono-size: clamp(1.05rem, 1.65vw, 1.43rem) !important;
    }
  `,
});
